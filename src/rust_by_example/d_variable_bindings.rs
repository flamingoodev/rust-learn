...
fn variable_scope() {
    // 此绑定生存于 variable_scope 函数中
    let long_lived_binding = 1;

    // 这是一个代码块，比 variable_scope 函数拥有更小的作用域
    {
        // 此绑定只存在于本代码块
...
fn variable_declare() {
    ...
    // 报错！使用了未初始化的绑定
    // error: use of possibly-uninitialized `another_binding`
